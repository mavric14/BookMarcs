...
feature 'User sees all the bookmarks' do
  describe "Showing bookmarks" do 
    ...
    it "should show bookmarks of all users" do
     ...
     it "should show bookmarks of a category that has been cliked" do
      visit user_session_path
      fill_in 'Email', with: 'admin@example.com'
      fill_in 'Password', with: 'helloworld'     
      click_on('Sign in')
      click_on('#Test Driven')
      page.should have_content('http://documentation.mailgun.com/faqs.html#faq')
...
feature 'User likes a bookmark' do
  ...
  describe "Deliting a bookmark"  do 
    before(:each) do 
     message1 = FactoryGirl.build(:mail) 
     catagory = FactoryGirl.create(:category)   
     user1 = FactoryGirl.build(:user)
    ...
    it "should be removalble from personal page" do
      visit user_session_path
      fill_in 'Email', with: 'admin@example.com'
      fill_in 'Password', with: 'helloworld'     
      click_on('Sign in')
      click_link('My Bookmarks')
      click_link('remove')
      page.should have_no_content("http://steve.dynedge.co.uk/2010/11/29/receiving-test-driven-incoming-email-for-rails-3/")
    end
  end 
